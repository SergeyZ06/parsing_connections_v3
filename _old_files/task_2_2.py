...
# Class for representation yearly connections' loads as html document for Apache2.
class LoadToHTML22:
    def __init__(self, dict_settings):
        ...
        self.sqlite_date = cgi.FieldStorage().getvalue('dropdown_load_year')

    def create_html(self):
        # Loading html pattern.
        document_html = get_query(self.path_html_task_1_0)
        str_task = '<p><b>2) Определить дату (время) когда было максимальное количество активных подключений клиентов' \
                   ' и перечень активных клиентов, суммарная нагрузка на канал (Throughput) в этот момент.</b></p>' \
                   '\n<p>За месяц и за год.</p>'
